...
public partial class WriteTagPage : ContentPage
{
	public WriteTagPage()
	{
		InitializeComponent();
    ...
    {
        await ShowInfo("Tag written!");
    }

    private async void Current_OnTagDiscovered(ITagInfo tagInfo, bool format)
    {
        tagInfo.Records = new NFCNdefRecord[]
        {
            new NFCNdefRecord(){
                TypeFormat = NFCNdefTypeFormat.Mime,
                MimeType = "application/nl.scoutingweurt.TTGAME",
                Payload = NFCUtils.EncodeToByteArray(EntryBox.Text)
            }
        };
